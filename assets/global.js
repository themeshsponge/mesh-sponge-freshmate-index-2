...
novtheme.doAddOrRemoveWishlist = function() {
    ...
    $(document).off('click.addOrRemoveWishlist', iconWishLists).on('click.addOrRemoveWishlist', iconWishLists, function(e) {
        ...
        if (!self.hasClass('whislist-added')) {
            ...
            var image = self.parents('.item-product').find('.product__thumbnail').attr('srcset') ? self.parents('.item-product').find('.product__thumbnail').attr('srcset') : self.parents('.item-product').find('.product__thumbnail').attr('src');
        ...
        $('.section-video').each(function(){
            var winHeight = $(window).height(),
                ...
                if (currentPosition - offset_top < 0) {
                    ...
                    $(this).find('.itemY_parallax').css('transform', 'translateY('+ scrolled+'px)');
                } else {
...
novtheme.VerticalMenu = function() {
    ...
    function handleResize() {
        if ($(window).width() <= 991) {
            ...
            $('.parent--lv1').find('.site-nav__link--send').click(function(e) {
                e.stopPropagation();
            });
        } else {
            handleLargeScreen();
